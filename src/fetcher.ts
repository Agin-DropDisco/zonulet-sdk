...
import IZonuDexPair from './abis/IZonuDexPair.json'
import IZonuDexFactory from './abis/IZonuDexFactory.json'
...
export abstract class Fetcher {
  ...
  public static async fetchPairData(
    ...
    platform: RoutablePlatform = RoutablePlatform.ZONUDEX
  ): Promise<Pair> {
    invariant(tokenA.chainId === tokenB.chainId, 'CHAIN_ID')
    const address = Pair.getAddress(tokenA, tokenB, platform)
    const [reserves0, reserves1] = await new Contract(address, IZonuDexPair.abi, provider).getReserves()
    ...
    const swapFee = JSBI.BigInt(await new Contract(liquidityToken.address, IZonuDexPair.abi, provider).swapFee())
  ...
  public static async fetchSwapFee(
    ...
    return {
      fee: JSBI.BigInt(await new Contract(liquidityToken.address, IZonuDexPair.abi, provider).swapFee()),
      owner: await new Contract(FACTORY_ADDRESS[liquidityToken.chainId], IZonuDexFactory.abi, provider).feeToSetter()
    }
  }
  ...
  > {
    ...
    const factoryContract = new Contract(FACTORY_ADDRESS[liquidityTokens[0].chainId], IZonuDexFactory.abi, provider)
    const liquidityTokenContract = new Contract(liquidityTokens[0].address, IZonuDexPair.abi, provider)
    let calls = []
    calls.push({
  ...
  public static async fetchAllSwapFees(
    ...
    const factoryContract = new Contract(FACTORY_ADDRESS[chainId], IZonuDexFactory.abi, provider)
  ...
  public static async fetchProtocolFee(
    ...
    const factoryContract = await new Contract(FACTORY_ADDRESS[chainId], IZonuDexFactory.abi, provider)
