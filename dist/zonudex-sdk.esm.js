...
var STAKING_REWARDS_FACTORY_ADDRESS = (_STAKING_REWARDS_FACT = {}, _STAKING_REWARDS_FACT[ChainId.MAINNET] = '0x0000000000000000000000000000000000001234', _STAKING_REWARDS_FACT[ChainId.RINKEBY] = '0x709db16a6ef437197938256460c49183dc36ca4d', _STAKING_REWARDS_FACT[ChainId.HARMONY] = '0xa26bD6052dbb86233465A01cA0518C6e62eEb9c0', _STAKING_REWARDS_FACT[ChainId.HARMONY_TESTNET] = '0x15b61cc2ac17e9826277b3ff41aa2644a774daab', _STAKING_REWARDS_FACT);
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider, platform) {
    try {
      ...
      return Promise.resolve(new Contract(address, IZonuDexPair.abi, provider).getReserves()).then(function (_ref) {
        ...
        return Promise.resolve(new Contract(liquidityToken.address, IZonuDexPair.abi, provider).swapFee()).then(function (_Contract$swapFee) {
          var swapFee = _BigInt.call(JSBI, _Contract$swapFee);

          var _BigInt2 = JSBI.BigInt;
          return Promise.resolve(new Contract(FACTORY_ADDRESS[tokenAmountA.token.chainId], IZonuDexFactory.abi, provider).protocolFeeDenominator()).then(function (_Contract$protocolFee) {
  ...
  Fetcher.fetchSwapFee = function fetchSwapFee(liquidityToken, provider) {
    try {
      ...
      return Promise.resolve(new Contract(liquidityToken.address, IZonuDexPair.abi, provider).swapFee()).then(function (_Contract$swapFee2) {
        var _BigInt3$call = _BigInt4.call(JSBI, _Contract$swapFee2);

        return Promise.resolve(new Contract(FACTORY_ADDRESS[liquidityToken.chainId], IZonuDexFactory.abi, provider).feeToSetter()).then(function (_Contract$feeToSetter) {
          return {
            fee: _BigInt3$call,
  ...
  Fetcher.fetchSwapFees = function fetchSwapFees(liquidityTokens, provider) {
    try {
      ...
      var factoryContract = new Contract(FACTORY_ADDRESS[liquidityTokens[0].chainId], IZonuDexFactory.abi, provider);
      var liquidityTokenContract = new Contract(liquidityTokens[0].address, IZonuDexPair.abi, provider);
      var calls = [];
      calls.push({
  ...
  Fetcher.fetchAllSwapFees = function fetchAllSwapFees(chainId, swapFeesCache, provider) {
    ...
    try {
      ...
      var factoryContract = new Contract(FACTORY_ADDRESS[chainId], IZonuDexFactory.abi, provider);
  ...
  Fetcher.fetchProtocolFee = function fetchProtocolFee(chainId, provider) {
    try {
      if (provider === undefined) provider = getDefaultProvider(getNetwork(chainId));
      return Promise.resolve(new Contract(FACTORY_ADDRESS[chainId], IZonuDexFactory.abi, provider)).then(function (factoryContract) {
...
//# sourceMappingURL=zonudex-sdk.esm.js.map
